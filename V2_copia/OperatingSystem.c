...
void moveToTheReadyStateDAEMON(int PID);
int Clock_IncreaseNumberOfClockInterrupts(); //ERROR 9/05
...
void OperatingSystem_MoveToTheREADYState(int PID) {	
	if(processTable[PID].queueID==DAEMONPROGRAM || PID==sipID){ //ERROR 27/04/24
		moveToTheReadyStateDAEMON(PID);
		return;
	}
...
void moveToTheReadyStateDAEMON(int PID){ //ERROR 27/04/24
	if(processTable[PID].state==NEW){
...
int OperatingSystem_ShortTermScheduler() {
	...
	if(selectedProcess==NOPROCESS){ //Error 27/04/2024
		return sipID;
	}
