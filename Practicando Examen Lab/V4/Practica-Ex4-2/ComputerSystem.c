...
heapItem *arrivalTimeQueue; //V3-1a
int numberOfProgramsInArrivalTimeQueue=0; //V3-1a
...
void ComputerSystem_PowerOn(int argc, char *argv[], int paramIndex) {
	...
	arrivalTimeQueue= Heap_create(PROCESSTABLEMAXSIZE); //V3-1a
	...
	while(!(numberOfNotTerminatedUserProcesses==0 && numberOfProgramsInArrivalTimeQueue==0)){ // NEW condition V3-4a
		Processor_DeactivatePSW_Bit(POWEROFF_BIT);
		Processor_InstructionCycleLoop();
