...
void OperatingSystem_ReducePriorityBlocked(){
	...
	for (i=0; i<PARTITIONTABLEMAXSIZE;i++){
		...
		if(pid>=0 && pid<PROCESSTABLEMAXSIZE && pid!=sipID) {
			if(processTable[pid].state==BLOCKED){
				float percentage= ((float)processTable[pid].processSize)/((float)partitionsTable[i].size)*100;
				if(percentage>25){
					int priority =processTable[pid].priority;
					if(priority-5>=0){
						processTable[pid].priority=priority-5;
						//ComputerSystem_DebugMessage(TIMED_MESSAGE,146,EXAM,0,"a",0,0,0,0.0);
...
void OperatingSystem_HandleClockInterrupt(){ //In OperatingSystem.c Exercise 1-b of V2   V2-1b
	...
	OperatingSystem_ReducePriorityBlocked();//Examen viejo V4
