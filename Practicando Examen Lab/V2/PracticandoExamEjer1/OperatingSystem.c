...
// and OS code and the system stack occupies 60 positions 
	...
	//processTable[sipID].queueID=DAEMONSQUEUE; //FIXED ERROR
	//if (numberOfSuccessfullyCreatedProcesses==1){
	if(numberOfNotTerminatedUserProcesses==0){ //V1-15
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	if (loadingPhysicalAddress==TOOBIGPROCESS) //V1-7a
...
void OperatingSystem_HandleException() {
	...
	OperatingSystem_TerminateExecutingProcess();
	OperatingSystem_PrintStatus(); //V2-3d ERROR
}
...
void OperatingSystem_Ex1(){ //EXAM-2023
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		case SYSCALL_YIELD: { //V1-12
			...
			int noReadyToRun=1; //EXAM 2023
			int noPriority=1; //EXAM 2023
			int i;
			for (i=0; i<PROCESSTABLEMAXSIZE;i++){
				if(i!=executingProcessID && processTable[i].queueID==queueID && processTable[i].state==READY){ //EXAM 2023
					noReadyToRun=0;
				}
				if(i!=executingProcessID && processTable[i].queueID==queueID && processTable[i].state==READY && processTable[i].priority==priority){ //EXAM 2023
					noPriority=0;
				}
			...
			if (nextProcessToRunPID==-1){
				...
				yieldFailedNoProcesses+=noReadyToRun; //EXAM 2023
				yieldFailedNoPriority+=noPriority; //EXAM 2023
			} else{
		...
		case SYSCALL_SLEEP: {//V2-5f
			...
			OperatingSystem_PrintStatus(); //V2-5g ERROR
			Processor_DeactivatePSW_Bit(INTERRUPT_MASKED_BIT); //NEW
...
void OperatingSystem_MoveToTheBLOCKEDState(int PID) { //V2-5f	
	...
	if (Heap_add(PID,sleepingProcessesQueue,QUEUE_WAKEUP, &(numberOfSleepingProcesses))>=0) { //Change queue type
...
void OperatingSystem_PrintReadyToRunQueue() { //V1-9a
	...
	ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,113,SHORTTERMSCHEDULE); 
		//for (i=0; i<numberOfReadyToRunProcesses[NUMBEROFQUEUES];i++) //NUMBEROFQUEUES //ALLPROCESSESQUEUE PROCESSTABLEMAXSIZE
		for (i=0; i<PROCESSTABLEMAXSIZE;i++){ 
			if(processTable[i].state==READY && processTable[i].queueID==DAEMONPROGRAM) { //V1-11b
				//ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,108,SHORTTERMSCHEDULE,readyToRunQueue[i]->info,processTable[readyToRunQueue[i]->info].priority);
				ComputerSystem_DebugMessage(NO_TIMED_MESSAGE,108,SHORTTERMSCHEDULE,i,processTable[i].priority);	  
			}
		}
